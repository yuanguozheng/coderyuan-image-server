...
class ImageResolver extends BaseService {
    ...
    handleRequest(req, res) {
        ...
        for (let index = 0; index < supportedExts.length; index++) {
            ...
            if (fs.existsSync(fullCompressedFilePath) && fs.statSync(fullCompressedFilePath).size > 0) {
        ...
        if (userAgent && BrowserUtils.isSafari(userAgent) && BrowserUtils.isSupportHeic(userAgent)) {
            ...
            if (fs.existsSync(heicPath) && fs.statSync(heicPath).size > 0) {
        ...
        if (fs.existsSync(fullNormalFilePath) && fs.statSync(fullNormalFilePath).size > 0) {
